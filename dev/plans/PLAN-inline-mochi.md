# Inline mochi-mqtt-server into monster-mq-edge

## Status (2026-04-28)

Not yet executed. Edge still depends on `github.com/mochi-mqtt/server/v2
v2.7.9` from `go.mod`. The HEAD SHA cited below (`5b7f94b`) was current
when the plan was drafted; before running step 1 re-pin to the current
upstream HEAD and update the SHA in the README-mochi.md note (step 5).
...
Strategy: **fork the source into `monster-mq-edge/internal/mqtt/`** (Strategy C in
...
```
monster-mq-edge/internal/mqtt/                 ← from mochi root
...
2. **Copy the source tree**
   ```bash
   DEST=/Users/vogler/Workspace/monster-mq-edge/internal/mqtt
   mkdir -p $DEST
   cd /Users/vogler/Workspace/mochi-mqtt-server
...
6. **Update our 5 broker files**
   - `monster-mq-edge/internal/broker/server.go`
   - `monster-mq-edge/internal/broker/hook_auth.go`
   - `monster-mq-edge/internal/broker/hook_storage.go`
   - `monster-mq-edge/internal/broker/hook_queue.go`
   - `monster-mq-edge/internal/broker/tls.go` *(if it imports anything mochi)*
...
7. **Detach the external dependency**
   ```bash
   cd /Users/vogler/Workspace/monster-mq-edge
