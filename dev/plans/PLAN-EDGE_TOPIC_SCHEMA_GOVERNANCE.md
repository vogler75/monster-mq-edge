...
When a feature is disabled, the Kotlin GraphQL resolvers keep the schema available but return empty/null/false results or a structured error result, depending on the declared return type.
...
- `contentType`, `version`, `description`, `examples`, and `tags` are optional.
...
The Kotlin broker validates publishes after ACL authorization and before message delivery.

The edge broker should use the same order:
...
The current Kotlin broker only implements `REJECT` behavior even though `enforcementMode` is stored as a string. The edge broker should preserve that behavior first. Additional modes such as `WARN` or `REPORT` can be added later behind explicit product decisions.
...
- Compile every `TopicSchema-Policy` JSON Schema.
- Load enabled `TopicNamespace` configs.
- Resolve `schemaPolicyName` to a compiled policy.
- Insert namespace bindings into an MQTT topic tree.
- Atomically swap the old tree for the new tree.
...
```text
internal/governance/
  model.go             # Policy, namespace, validation result structs
  validator.go         # JSON Schema compilation and payload validation
  topic_matcher.go     # MQTT topic filter matching/tree
  cache.go             # reloadable immutable cache
  graphql.go           # resolvers, if GraphQL is colocated by feature
  mqtt_hook.go         # publish-path enforcement adapter
```
...
- Namespace name conflicts and policy name conflicts should return mutation `errors`, not GraphQL transport errors.
...
The Kotlin cache uses the same `TopicTree` matching used for MQTT subscriptions and returns the first matching namespace entry. The Go edge broker should make this deterministic.

Implementation options:

1. Reuse the edge broker's existing MQTT subscription topic matcher if it supports retrieving bound data.
2. Build a governance-specific topic filter tree.
3. Use a simple list scan for the first version if namespace counts are expected to be small.

For compatibility and future scale, prefer option 1 or 2. If list scan is used initially, document ordering and add tests before replacing it with a tree.
...
- Add MQTT publish tests:
  - publish to ungoverned topic succeeds
  - publish valid governed JSON succeeds
  - publish invalid governed JSON is rejected
  - disabled namespace does not enforce
  - namespace referencing deleted/missing policy is ignored after reload
- Add schema compatibility checks against `broker/src/main/resources/schema-topic-schema.graphqls`.

## Edge-Specific Constraints

Keep the edge implementation lightweight:
